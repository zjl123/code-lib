...
+ (NSMutableDictionary *)sortedArrayWithPinYinDic:(NSArray *)userList {
  ...
  for (NSString *key in _keys) {
    ...
    if ([user isMemberOfClass:[IMUserModel class]]) {
      IMUserModel *userInfo = (IMUserModel*)user;
      if (userInfo.remarksName.length > 0 && ![userInfo.remarksName isEqualToString:@""]) {
        firstLetter = [self getFirstUpperLetter:userInfo.remarksName];
      } else {
