...
- (void)viewDidLoad {
   ...
   // NSString *verson = ;
    ...
    self.currentVerson.text = ZGS(@"currentV");
    ...
    NSString *eq = appinfo[@"eq"];
    if(eq.length > 0)
    {
        NSURL *url = [NSURL URLWithString:@"eq"];
        [_QRImage sd_setImageWithURL:url placeholderImage:[UIImage imageNamed:@"gray-mc"]];
    }
    else
    {
        _QRImage.hidden = YES;
