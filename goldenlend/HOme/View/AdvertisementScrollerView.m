	...
	if (self) {
        ...
        for (int i=0; i<pageCount; i++) {
          ...
          //  [imgView setImageWithURL:[NSURL URLWithString:imgURL]placeholderImage:IMAGE_WITH_NAME(@"过度.png")];
                [imgView sd_setImageWithURL:[NSURL URLWithString:imgURL] placeholderImage:[UIImage imageNamed:@"a"]];
            }
            else
            {
...
-(void)timerAction
{
    ...
    if (self.contentOffset%(int)width1!=0)
    {
    ...
    else
    {
        ...
        if (self.contentOffset == width1)
        {
         
            self.contentOffset+=width1;
            [timerTwo setFireDate:[NSDate distantPast]];//开启定时器
            [timerOne setFireDate:[NSDate distantFuture]];//关闭定时器
            
        }else if(MyScrollView.contentOffset.x>=width1*2&&MyScrollView.contentOffset.x<width1*([imageArray count]-1)){
            
            self.contentOffset+=width1;
            
            
        }else if(MyScrollView.contentOffset.x == width1*([imageArray count]-1))
        {
            
            [MyScrollView setContentOffset:CGPointMake(width1, 0)];
